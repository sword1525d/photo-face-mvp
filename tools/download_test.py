...
def main() -> int:
    ...
    response_a = client.get(f"/evento/{slug}/foto/{photo_a}/baixar")
    response_thumb = client.get(f"/storage/{photos.get(photo_a)['thumbnail_path']}")
    photo_b = upload("CORRIDA 01.JPG", jpeg_b)
    photo_upload_payload = client.get(f"/evento/{slug}/foto/{photo_b}/baixar").data
    ...
    check("HTTP 200", response_a.status_code == 200, f"-> {response_a.status_code}")
    check(
        "conteúdo idêntico ao enviado",
        response_a.data == jpeg_a,
        f"({len(response_a.data)} bytes)",
    )
    check("mime de imagem", response_a.headers.get("Content-Type", "").startswith("image/jpeg"))
    disposition = response_a.headers.get("Content-Disposition", "")
    ...
    check(
        ...
        response_thumb.status_code == 200
        and response_thumb.data != response_a.data
        and len(response_thumb.data) < len(response_a.data),
        f"{len(response_thumb.data)} < {len(response_a.data)} bytes",
    )
    check("a segunda foto também veio inteira", photo_upload_payload == jpeg_b)

    print("[4/7] RAW: o JPEG é o original legível, o .nef é opt-in")
    raw_default = client.get(f"/evento/{slug}/foto/{photo_raw}/baixar")
    raw_explicit = client.get(f"/evento/{slug}/foto/{photo_raw}/baixar?raw=1")
    check("sem ?raw=1 vem o JPEG (nome .jpg)", raw_default.data == jpeg_from_raw)
    check("nome do arquivo sem ?raw=1", "DSC_0042.jpg" in raw_default.headers.get("Content-Disposition", ""))
    check("com ?raw=1 vem o .nef preservado", raw_explicit.data == nef_bytes)
    check("nome do arquivo com ?raw=1", "DSC_0042.nef" in raw_explicit.headers.get("Content-Disposition", ""))

    print("[5/7] Download de VÁRIAS fotos (ZIP)")
    response_zip = client.post(
        f"/evento/{slug}/baixar", data={"ids": [str(photo_a), str(photo_b), str(photo_raw)]}
    )
    check("HTTP 200", response_zip.status_code == 200, f"-> {response_zip.status_code}")
    check("mime de zip", response_zip.headers.get("Content-Type") == "application/zip")
    check("nome do zip", f"{slug}-3-fotos.zip" in response_zip.headers.get("Content-Disposition", ""))
    if response_zip.status_code == 200:
        with zipfile.ZipFile(io.BytesIO(response_zip.data)) as archive:
    ...
    single = client.post(f"/evento/{slug}/baixar", data={"ids": [str(photo_a)]})
    check(
        "seleção com 1 foto não vira ZIP",
        single.status_code == 200
        and single.data == jpeg_a
        and single.headers.get("Content-Type", "").startswith("image/jpeg"),
    )

    print("[6/7] Erros devem ser amigáveis (nada de traceback / 500)")
    empty = client.post(f"/evento/{slug}/baixar", data={}, headers=JSON_HEADERS)
    check(
        "sem seleção -> 400 com mensagem",
        empty.status_code == 400 and "Selecione" in (empty.get_json() or {}).get("error", ""),
        f"-> {empty.get_json()}",
    )

    other = events.create("Outro evento", "")
    other_photo = upload("OUTRO.JPG", make_jpeg((10, 10, 10)), target=int(other["id"]))
    foreign = client.post(f"/evento/{slug}/baixar", data={"ids": [str(other_photo)]}, headers=JSON_HEADERS)
    check(
        "id de outro evento é ignorado",
        foreign.status_code == 400,
        f"-> {foreign.status_code} {foreign.get_json()}",
    )
    foreign_single = client.get(f"/evento/{slug}/foto/{other_photo}/baixar")
    check("id de outro evento no link direto -> 404", foreign_single.status_code == 404, f"-> {foreign_single.status_code}")
    missing_id = client.get(f"/evento/{slug}/foto/999999/baixar")
    check("id inexistente -> 404", missing_id.status_code == 404, f"-> {missing_id.status_code}")

    (originals / Path(photos.get(photo_a)["original_path"]).name).unlink()
    gone = client.get(f"/evento/{slug}/foto/{photo_a}/baixar", headers=JSON_HEADERS)
    check(
        "arquivo sumiu do disco -> mensagem, não 500",
        gone.status_code == 404 and "disponível" in (gone.get_json() or {}).get("error", ""),
        f"-> {gone.get_json()}",
    )
    (originals / Path(photos.get(photo_a)["original_path"]).name).write_bytes(jpeg_a)
